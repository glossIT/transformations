...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <xsl:template name="coords" match="p:TextRegion | p:TextLine">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains(./@id, '_textblock_') and not(contains(./@custom, 'Numbering'))">
            ...
            <xsl:when test="contains(./@id, '_textblock_') and contains(./@custom, 'Numbering')">
                <zone xmlns="http://www.tei-c.org/ns/1.0" ulx="{substring-before($Xmin, ',')}" uly="{substring-before($Ymin, ',')}"
                    lrx="{substring-before($Xmax, ',')}" lry="{substring-before($Ymax, ',')}"
                    points="{concat($XYmin2, ' ', $XmaxYmin, ' ', $XYmax2, ' ', $XminYmax)}"  xml:id="{$ID}" type="{$regiontype}">
                    <xsl:apply-templates/>
                </zone>
            </xsl:when>
            <!-- for Text_lines --> 
            <xsl:when test="contains(./@id, '_line_') and not(contains(./@custom, 'gloss')) and not(contains(./@custom, 'Reference')) and not(contains(parent::p:TextRegion[1]/@custom, 'Numbering')) and not(contains(./@custom, 'Heading'))">
                <zone type="textline"                
            ...
            <xsl:when test="contains(./@id, '_line_') and (contains(./@custom, 'gloss') or contains(./@custom, 'Reference'))">
                <zone type="gloss"                
                    xml:id="{concat($ID,'_gloss')}">
                    <xsl:attribute name="rendition" select="substring-before(substring-after(./@custom, 'structure {type:' ), ';}')"/>
                        <line ulx="{substring-before($b-Xmin, ',')}"
                        uly="{substring-before($b-Ymin, ',')}" 
                        lrx="{substring-before($b-Xmax, ',')}" 
                        lry="{substring-before($b-Ymax, ',')}" 
            ...
            <xsl:when test="contains(./@id, '_line_') and contains(parent::p:TextRegion[1]/@custom, 'Numbering')">
                <zone type="numbering">                               
                    ...
                    <line ulx="{substring-before($b-Xmin, ',')}"
                        ...
                        xml:id="{$ID}"
                        >  
                        <xsl:attribute name="rendition" select="concat(substring-before(substring-after(parent::p:TextRegion[1]/@custom, 'structure {type:NumberingZone:' ), ';}'), '-number')"/>
                        <xsl:apply-templates/></line> </zone>
            </xsl:when>
            <xsl:when test="contains(./@id, '_line_') and contains(./@custom, 'HeadingLine')">
                <zone type="textline"                
                    xml:id="{concat($ID,'_heading')}" 
                    rendition="heading'">
                    <line ulx="{substring-before($b-Xmin, ',')}"
                        uly="{substring-before($b-Ymin, ',')}" 
                        lrx="{substring-before($b-Xmax, ',')}" 
